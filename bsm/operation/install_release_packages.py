...
class InstallReleasePackages(Base):
                    ...
                    for version, value in self._prop['packages_install'][category][subdir][package].items():
                        ...
                        for pkg_dep in pkg_deps:
                            ...
                            for dep in package_steps[pkg_dep]:
                                dep_end_index = len(self._prop['packages_install'][dep[0]][dep[1]][dep[2]][dep[3]]['step'][atomic_end]) - 1
    ...
    def __save_release_status(self, start_time, end_time):
        ...
        self._prop['release_status']['option'] = {}
        option_from_install = ensure_list(
            self._prop['release_setting'].get('option_from_install', []))
        for opt in option_from_install:
            if opt in self._prop['option_release']:
                self._prop['release_status']['option'][opt] = self._prop['option_release'][opt]
