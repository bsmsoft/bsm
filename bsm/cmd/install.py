...
from bsm.config.config_release import ConfigReleaseTransformError
...
def _install(config, config_version, source, force, yes, no_clean):
    ...
    install = BsmInstall(config, config_version, None, transformer)
...
def _clean_only(config, config_version):
    ...
    install = BsmInstall(config, config_version, config_release, transformer)
    install.install_packages()
