    ...
    def __init__(self, config):
        ...
        if not ('version' in config['scenario'] and config['scenario']['version']):
            raise ReleasePathError(
                '"version" not specified in config scenario')

        self['main_dir'] = os.path.join(
            config['release_dir'], config['scenario']['version'])
        ...
        self['config_package_dir'] = os.path.join(
            self['content_dir'], 'package')
        self['handler_dir'] = os.path.join(self['content_dir'], 'handler')

        self['handler_python_dir'] = os.path.join(self['main_dir'], 'handler')
        self['handler_module_dir'] = os.path.join(
            self['main_dir'], 'handler', HANDLER_MODULE_NAME)
