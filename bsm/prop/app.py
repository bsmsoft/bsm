...
class App(CommonDict):
    ...
    def __set_default(self):
        ...
        self.setdefault('bsm_info_file', '~/.'+app_id+'.info')
        self.setdefault('release_ctrl_dir', '.bsm')
        self.setdefault('category_ctrl_dir', '.bsm')
        self.setdefault('package_prop_file', 'package.yml')
        self.setdefault('env', {})
        ...
        if 'example_config_user' in self and 'app_root' in self:
            self['example_config_user'] = os.path.join(
                self['app_root'], self['example_config_user'])
        else:
